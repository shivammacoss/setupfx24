    ...
    try {
      ...
      if (user && user.email) {
        ...
        await sendTemplateEmail('kyc_submitted', user.email, {
          ...
          platformName: settings?.platformName || 'SetupFX24',
          supportEmail: settings?.supportEmail || 'support@SetupFX24.com',
    ...
    try {
      if (user && user.email) {
        ...
        const emailResult = await sendTemplateEmail('kyc_approved', user.email, {
          ...
          platformName: settings?.platformName || 'SetupFX24',
          loginUrl: settings?.loginUrl || 'https://SetupFX24.com/login',
          supportEmail: settings?.supportEmail || 'support@SetupFX24.com',
          year: new Date().getFullYear().toString()
        })
