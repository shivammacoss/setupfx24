# SetupFX24 VPS Deployment Guide (Fresh OS Installation)
...
scp -r ./nalmi SetupFX24@YOUR_VPS_IP:/home/SetupFX24/
```

### Option C: Using SFTP/FileZilla
- Connect to VPS using FileZilla
- Upload the `nalmi` folder to `/home/SetupFX24/`

## Step 9: Configure Backend

```bash
cd /home/SetupFX24/nalmi/backend
...
PORT=5001
NODE_ENV=production
MONGODB_URI=mongodb://localhost:27017/SetupFX24
...
```bash
cd /home/SetupFX24/nalmi/frontend
...
# Start with PM2
pm2 start server.js --name SetupFX24-backend
...
npm install -g serve
cd /home/SetupFX24/nalmi/frontend
pm2 start "serve -s dist -l 5173" --name SetupFX24-frontend
```
...
# Configure nginx
sudo nano /etc/nginx/sites-available/SetupFX24
```
...
server {
    ...
    # Frontend
    location / {
        root /home/SetupFX24/nalmi/frontend/dist;
...
Enable the site:
```bash
sudo ln -s /etc/nginx/sites-available/SetupFX24 /etc/nginx/sites-enabled/
sudo nginx -t
...
pm2 stop SetupFX24-backend   # Stop backend
pm2 delete SetupFX24-backend # Remove from PM2
```
...
### View logs
```bash
pm2 logs SetupFX24-backend --lines 100
```
